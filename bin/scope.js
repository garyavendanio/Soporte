const {glob, minify, read, write, validClassName} = require('./util');
...
if (argv.h || argv.help) {
    console.log(`
        usage: scope.js [-s{cope}=your_great_new_scope_name][cleanup]

        example:
	        scope.js // will scope with x-scope
...
function getNewScope() {
    ...
    const scopeFromInput = argv.scope || argv.s || 'x-scope';
    ...
    await Promise.all(
        files.map(async store => {
            try {
                ...
                // const output = await renderApp(`.${scope} {\n${stripComments(store.data)}\n}`);
                store.data = `/* scoped: ${scope} */\n${
                    output
                        .replace(new RegExp(`.${scope} ${/{(.|[\r\n])*?}/.source}`), '')
                        .replace(new RegExp(`.${scope}${/\s((\.(x-(drag|modal-page|offcanvas-page|offcanvas-flip)))|html)/.source}`, 'g'), '$1')
                }`;
...
const currentScopeLegacyRe = /\.(x-scope)/;
