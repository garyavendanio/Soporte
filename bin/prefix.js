...
if (argv.h || argv.help) {
    console.log(`
        usage: prefix.js [-p{refix}=your_great_new_prefix]

        example:
	        prefix.js // will prompt for a prefix to replace the current one with
	        prefix.js -p=xyz // will replace any existing prefix with abc
...
function findExistingPrefix() {
    ...
    allFiles.filter(({file}) => ~file.indexOf('app.css')).some(({file, data}) => {
            ...
            files.forEach(file =>
                reads.push(util.read(file, data =>
                    allFiles.push({
                        file,
                        data,
                        replace: (data, needle, replace) => data.replace(new RegExp(`${needle}-`, 'g'), `${replace}-`).replace(new RegExp(`(${needle})?app`, 'g'), `${replace === 'da' ? '' : replace}app`)
                    })
                ))
            );
            Promise.all(reads).then(res);
